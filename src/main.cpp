...
#if defined(NDEBUG)
# error "Umtcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x722e3c7f06c4d4169a00a3d57d8f83a629bc0fd196835d1d26fdddca784638bd");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Umtcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Umtcoin Signed Message:\n";
...
{
    // Umtcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 420000); // Umtcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3 * 24 * 60 * 60; // Umtcoin: 3.5 days
static const int64 nTargetSpacing = 2 * 60; // Umtcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Umtcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Umtcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x447a8d9640bc73ac6a298622d5627a8f4fcb68412bc15cedfc905bddac7afdc8");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Tue Mar 01 2022 17:35:26 GMT+0500 5th Day of Russia's offensive on Ukraine ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0475497de4fce511db9fa33c50d1d4134886608762466c7a667350107bfe4e9e9b04cf5eb54cb1b777a67bf7fcfda515fc465f3841f6aadbd3e52d9603c5e415ed") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1646137930;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2087964545;

        if (fTestNet)
        {
            block.nTime    = 1646137871;
            block.nNonce   = 385477870;
        }
...
if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xc6be944a0de57987f8b07a2e81cc2e2378a4cab65f2c02c71203e57040ad835a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc5, 0xb3, 0xdf }; // Umtcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("UmtcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("UmtcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static UmtcoinMiner(CWallet *pwallet)
{
    printf("UmtcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("umtcoin-miner");
    ...
    try { loop {
        ...
        printf("Running UmtcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("UmtcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&UmtcoinMiner, pwallet));
}
